...
!This code uses a hybrid of bisection and quadratic approximation to evaluate the eigen energies of the oddstates for the finite square well.
	...
	!the actual function for the odd states		
	function f(E)
		implicit none
		real(kind = 8):: f,E,alpha,beta
		real(kind = 8),parameter :: Vo = 10.0, z = 3.0, hbar_sq = 7.6199682, m = 1.0
		f = alpha(E)*cos(alpha(E)*z) + beta(E)*sin(alpha(E)*z)	
	end function
