...
! this script contains codes the evaluates the wave function at points of x to be used to plot the wave functions for oddstates
	...
	program oddsolution
		...
		x = -5.0
		E1 = 2.8213916080700518
		E2 = 9.8672038351564115
		
		!B = (C*exp(-beta(E)*a)/cos(alpha(E)*a))
		
		open(1, file = 'odd_solution')
...
! equation for the wave function for inside the well.	
	function psy2(x,E)
		...
		B = (-C*exp(-beta(Q)*a)/sin(alpha(Q)*a))
		
		psy2 = B*sin(alpha(E)*x)
	end function
